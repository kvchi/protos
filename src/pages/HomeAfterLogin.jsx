...
import { getBusinesses } from "../api/business.service";
      ...
      <div className="lg:px-24 lg:py-10 w-full">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-3 px-4 md:px-8 lg:px-24 py-10 w-full gap-5">
          {loadingBiz ? (
            ...
            businesses.slice(0, window.innerWidth < 1024 ? 6 : 9).map((biz) => (
              <BusinessCard
                key={biz.id}
                business={{
                  image:
                    biz.business_image || "https://via.placeholder.com/150",
                  title: biz.business_name,
                  rating: 4,
                  address:
                    biz.location?.[0]?.street_address ||
                    "Address not available",
                  features: biz.business_description || "",
                }}
              />
            ))
          )}
        ...
        <div className="px-4 md:px-10 lg:px-24 py-16">
          <h2 className="text-primary font-bold text-2xl mb-2">Latest News</h2>
